...
def scatter_yearcircle(X,Y,fsize=(6,6),title=None,xlabel=None,ylabel=None,axhline=None,axvline=None,\
            label=False,annotate=False,time=None,rng=None,dpi=100,color='b'):
    fm = plt.figure(figsize=fsize,dpi=dpi, facecolor='w')
    m = plt.gca()
    cm = m.scatter(X,Y,s=300,alpha=1,c='white',edgecolors=color,linewidth=1.5)
...
,vmin=None,vmax=None,title=None, barLoc='bottom',shrink=.8,colorbar=True,box=None,resolution=0.25,grid=None,ij=[1,1],pad=0.02\
    ,TRANSFORM=0 ,LANDCOLOR=True):
    m = plt.subplot(grid[ij[0],ij[1]],projection=projection(central_longitude=central_longitude))
    m.coastlines()
    cm = m.pcolormesh(XC, YC, data, transform=ccrs.PlateCarree(central_longitude=TRANSFORM), cmap=cmap, vmin=vmin, vmax=vmax)
    if LANDCOLOR==True: m.add_feature(cart.feature.LAND, color='grey', zorder=100, edgecolor='k')
        
    if colorbar==True:
        cax, kw = matplotlib.colorbar.make_axes(m, location=barLoc, pad=pad, shrink=shrink,aspect=40)
...
def contourfGridspec(XC,YC,data,projection=ccrs.PlateCarree,central_longitude=180,cmap='RdBu_r',form=None\
        ,levels=None,title=None, barLoc='bottom',shrink=1,colorbar=True,box=None,resolution=0.25,grid=None,ij=[1,1]):
    m = plt.subplot(grid[ij[0],ij[1]],projection=projection(central_longitude=central_longitude))
    m.coastlines()
    cm = m.contourf(XC, YC, data, transform=ccrs.PlateCarree(),levels=levels, cmap=cmap)
    ...
    if colorbar==True:
        ...
        b = plt.colorbar(cm, cax=cax, **kw,format=form) #        b.set_label(unit, fontsize=16, labelpad=-10, y=1.13, rotation=0)
        b.formatter.set_powerlimits((-5, 4)) ### 지수 단위 설정
        b.ax.tick_params(labelsize=10)
        b.ax.yaxis.get_offset_text().set(size=16)
    m.set_title(title, fontsize=17, fontweight = 'bold')
    if box!=None:
        BOX1,BOX2,BOX3,BOX4 = box[0]*resolution-180,box[1]*resolution-180,-box[2]*resolution+90,-box[3]*resolution+90
...
def timeseriesGridspec(X,data,year_str,title=None,grid=None,ij=[1,1],c='b',gridline=True,scatter=None,tick=2,\
                      name=None,axhline=None,axvline=None,multi=False,ylabel=None,xlabel=None,loc=2):
    ...
    if title!=None:
        m.set_title(title,size=18)
    if xlabel!=None:
        m.set_title(xlabel,size=15)
    m.legend(loc=loc,fontsize=12,frameon=False) ######
    if axhline!=None:
        m.axhline(axhline,c='k',alpha=0.3)
    if axvline!=None:
        m.axvline(axvline[0],c='r',ls='--',alpha=0.5)
        m.axvline(axvline[1],c='r',ls='--',alpha=0.5)
